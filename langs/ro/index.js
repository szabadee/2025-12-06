  ...
  postData(getCreditApiUrl, requestCredit)
    .then((data) => {
      ...
      if (dataCredit.credit_points >= 3500) {
        qualifiedMessage.innerHTML = `<span class="text-primary-color text-center mt-6 mb-6 lh-25"><strong>Felicitări, ai fost introdus în urna de extragere!</strong></span>`;
      }
    })
    .catch((error) => {
    ...
    .then((data) => {
      ...
      switch (dataRegist.status_id) {
        case 2:
          ...
          <div class="col-lg-4">
            <button type="submit" onclick="getActionName(this.id)" id="createRegBtn" class="btn btn-create">Trimite înregistrarea</button>
          </div>

          `;
          regStatus.innerText = "Nu te-ai înregistrat încă";
          guestName.classList.remove("form-control-second");
          guestName.classList.add("form-control-first");
          guestBoxInfo.innerHTML = `<span class="fs-16 lh-22"><sup>*</sup>Se poate introduce doar un nume</span>`;
          gdpr.innerHTML = `
          
          <p class="small-font">La locația evenimentului este necesară acceptarea <strong>Declarației</strong> privind protecția datelor,
          în legătură cu utilizarea imaginilor, sunetelor și înregistrărilor video realizate în timpul evenimentului,
          pe care o poți citi în prealabil făcând<a href="docs/nics-adatkezelesi-tajekoztato.pdf" target="_blank"> clic aici.</a></p>
          `;
          break;
        case 1:
          regActions.innerHTML = `
            ...
            <div class="">
              <button type="submit" onclick="getActionName(this.id)" id="updateRegBtn" class="btn btn-update">Actualizează înregistrarea</button>
            </div>
            <br>
            <hr>
            <span>Dacă din orice motiv nu poți participa la petrecere, te rugăm să îți ștergi înregistrarea – ne vei ajuta mult în pregătiri!</span>
            <div class="pt-20">
              <button type="submit" onclick="getActionName(this.id)" id="deleteRegBtn" class="btn btn-delete">Șterge înregistrarea</button>
            </div>
          ...
          regStatus.innerText = "Ai trimis deja înregistrarea ta";
          guestBoxInfo.innerHTML = `<span class="fs-16"><sup>*</sup>Numele poate fi introdus, șters ulterior</span>`;
          guestNameLabel.innerText = "Numele invitatului:";
        ...
        case 0:
          ...
          customerName.innerText = "Identificarea a eșuat!";
          regForm.innerHTML = ``;
        ...
        case 5:
          ...
          closedRegBox.innerHTML = `
            ...
            <span class="material-icons text-primary-color" id="regIcon">lock</span>
              <span><strong>Formularul de înscriere pentru petrecerea aniversară a fost deja închis!</strong><br>
              <span class="d-block pt-10">Dacă te-ai înregistrat, ne vedem la petrecere! Dacă nu poți veni acum, nu fi trist – ne vom revedea și în iunie 2026!
...
form.addEventListener("submit", (event) => {
  ...
  if (guestName.value.trim() !== "" && !isValidText(guestName.value)) {
    ...
    guestErrorMessage.innerHTML = `<label class"d-block pt-10 fs-16">Te rugăm să introduci doar litere!</label>`;
  } else {
      ...
      postData(createRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Mulțumim, te-ai înregistrat cu succes!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>În câteva secunde îți vom actualiza înregistrarea, <strong>pe care o poți verifica, modifica sau șterge pe această pagină.</strong></i>
            </div>
            
              `;
            document.getElementById("createRegBtn").disabled = true;
          } else {
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">A apărut o eroare la înregistrare!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(updateRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Ai modificat înregistrarea cu succes!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">A apărut o eroare la modificarea înregistrării!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(deleteRegistAPiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Ai șters înregistrarea cu succes!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>În câteva secunde îți vom actualiza starea înregistrării, <strong>pe care o poți verifica sau reînregistra dacă este necesar.</strong></i>
            </div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">A apărut o eroare la ștergerea înregistrării!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
