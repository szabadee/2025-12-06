  ...
  postData(getCreditApiUrl, requestCredit)
    .then((data) => {
      ...
      if (dataCredit.credit_points >= 3500) {
        qualifiedMessage.innerHTML = `<span class="text-primary-color text-center mt-6 mb-6 lh-25"><strong>Честитамо, учествује у наградној игри!</strong></span>`;
      }
    })
    .catch((error) => {
    ...
    .then((data) => {
      ...
      switch (dataRegist.status_id) {
        case 2:
          ...
          <div class="col-lg-4">
            <button type="submit" onclick="getActionName(this.id)" id="createRegBtn" class="btn btn-create">Слање регистрације</button>
          </div>

          `;
          regStatus.innerText = "Још се ниси пријавио/ла";
          guestName.classList.remove("form-control-second");
          guestName.classList.add("form-control-first");
          guestBoxInfo.innerHTML = `<span class="fs-16 lh-22"><sup>*</sup>Могуће је дати само једно име</span>`;
          gdpr.innerHTML = `
          
          <p class="small-font">На месту одржавања прославе, потребно је прихватање наше <strong>Обавештења о заштити података</strong> у вези са коришћењем слика,
          аудио и видео снимака направљених на догађају, коју можете унапред прочитати <a href="docs/nics-adatkezelesi-tajekoztato.pdf" target="_blank">кликом овде.</a></p>
          `;
          break;
        case 1:
          regActions.innerHTML = `
            ...
            <div class="">
              <button type="submit" onclick="getActionName(this.id)" id="updateRegBtn" class="btn btn-update">Ажурирање регистрације</button>
            </div>
            <br>
            <hr>
            <span>Ако из неког разлога ипак не можеш доћи на прославу, молим те избриши своју регистрацију, како би нам помогао у припреми!</span>
            <div class="pt-20">
              <button type="submit" onclick="getActionName(this.id)" id="deleteRegBtn" class="btn btn-delete">Брисање регистрације</button>
            </div>
          ...
          regStatus.innerText = "Већ си послао пријаву";
          guestBoxInfo.innerHTML = `<span class="fs-16"><sup>*</sup>Име је могуће накнадно додати, брисати</span>`;
          guestNameLabel.innerText = "Име твог госта:";
        ...
        case 0:
          ...
          customerName.innerText = "Неуспешна идентификација!";
          regForm.innerHTML = ``;
        ...
        case 5:
          ...
          closedRegBox.innerHTML = `
            ...
            <span class="material-icons text-primary-color" id="regIcon">lock</span>
              <span><strong>Платформа за регистрацију на рођенданску свечаност је већ затворена!</strong><br>
              <span class="d-block pt-10">Ако се региструјете, видимо се на прослави! Не буди тужан ако сада не можеш да присуствујеш, имаћеш прилике у јуну 2026. да славиш са нама!
...
form.addEventListener("submit", (event) => {
  ...
  if (guestName.value.trim() !== "" && !isValidText(guestName.value)) {
    ...
    guestErrorMessage.innerHTML = `<label class"d-block pt-10 fs-16">Молим те, користи само слова!</label>`;
  } else {
      ...
      postData(createRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Хвала на регистрацији!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>Ажурираћемо твоју регистрацију за неколико секунди, <strong>коју можеш проверити, мењати или избрисати на овој платформи.</strong></i>
            </div>
            
              `;
            document.getElementById("createRegBtn").disabled = true;
          } else {
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Грешка приликом регистрације!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(updateRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Успешно си изменуо/ла регистрацију!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">грашка приликом измене регистрације!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(deleteRegistAPiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Успешно си избрисао/ла регистрацију!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>Ажурираћемо твоју регистрацију за неколико секунди, <strong>коју можеш проверити, или се можеш поново пријавити ако је потребно.</strong></i>
            </div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Грешка приликом брисања регистрације!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
