  ...
  postData(getCreditApiUrl, requestCredit)
    .then((data) => {
      ...
      if (dataCredit.credit_points >= 3500) {
        qualifiedMessage.innerHTML = `<span class="text-primary-color text-center mt-6 mb-6 lh-25"><strong>Gratulujeme, bol si zaradený do žrebovania!</strong></span>`;
      }
    })
    .catch((error) => {
    ...
    .then((data) => {
      ...
      switch (dataRegist.status_id) {
        case 2:
          ...
          <div class="col-lg-4">
            <button type="submit" onclick="getActionName(this.id)" id="createRegBtn" class="btn btn-create">Odoslať registráciu</button>
          </div>

          `;
          regStatus.innerText = "Zatiaľ si sa nezaregistroval";
          guestName.classList.remove("form-control-second");
          guestName.classList.add("form-control-first");
          guestBoxInfo.innerHTML = `<span class="fs-16 lh-22"><sup>*</sup>Môžeš zadať iba jedno meno</span>`;
          gdpr.innerHTML = `
          
          <p class="small-font">Na mieste konania bude potrebné prijať naše Zásady ochrany osobných údajov, týkajúce sa použitia obrazových,
          zvukových a videozáznamov zhotovených počas podujatia, ktoré si môžeš prečítať <a href="docs/nics-adatkezelesi-tajekoztato.pdf" target="_blank">kliknutím sem.</a></p>
          `;
          break;
        case 1:
          regActions.innerHTML = `
            ...
            <div class="">
              <button type="submit" onclick="getActionName(this.id)" id="updateRegBtn" class="btn btn-update">Aktualizovať registráciu</button>
            </div>
            <br>
            <hr>
            <span>Ak sa z akéhokoľvek dôvodu nemôžeš zúčastniť oslavy, prosíme ťa, <strong>zruš svoju registráciu, veľmi nám tým pomôžeš pri prípravách!</span>
            <div class="pt-20">
              <button type="submit" onclick="getActionName(this.id)" id="deleteRegBtn" class="btn btn-delete">Zrušiť registráciu</button>
            </div>
          ...
          regStatus.innerText = "Už si odoslal svoju registráciu";
          guestBoxInfo.innerHTML = `<span class="fs-16"><sup>*</sup>Meno je možné zadať neskôr, aj ho vymazať</span>`;
          guestNameLabel.innerText = "Meno hosťa:";
          regIcon.innerText = "check_circle";
          gdpr.innerHTML = `
      
          <p class="small-font">Na mieste konania bude potrebné prijať naše <strong>Zásady ochrany osobných údajov, týkajúce sa použitia obrazových,
          zvukových a videozáznamov zhotovených počas podujatia, ktoré si môžeš prečítať <a href="docs/nics-adatkezelesi-tajekoztato.pdf" target="_blank">kliknutím sem.</a></p>
        ...
        case 0:
          ...
          customerName.innerText = "Identifikácia zlyhala!";
          regForm.innerHTML = ``;
        ...
        case 5:
          ...
          closedRegBox.innerHTML = `
            ...
            <span class="material-icons text-primary-color" id="regIcon">lock</span>
              <span><strong>Prihlasovací formulár k narodeninovému podujatiu je už uzavretý!</strong><br>
              <span class="d-block pt-10">Ak si sa zaregistroval, vidíme sa na oslave! Ak teraz nemôžeš prísť, nezúfaj – v júni 2026 bude opäť možnosť oslavovať spolu!
...
form.addEventListener("submit", (event) => {
  ...
  if (guestName.value.trim() !== "" && !isValidText(guestName.value)) {
    ...
    guestErrorMessage.innerHTML = `<label class"d-block pt-10 fs-16">Zadaj prosím iba písmená!</label>`;
  } else {
      ...
      postData(createRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Ďakujeme, úspešne si sa zaregistroval!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>O niekoľko sekúnd aktualizujeme tvoju registráciu, ktorú môžeš skontrolovať, <strong>upraviť alebo vymazať na tejto stránke.</strong></i>
            </div>
            
              `;
            document.getElementById("createRegBtn").disabled = true;
          } else {
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Pri registrácii nastala chyba!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(updateRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Úspešne si upravil svoju registráciu!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Pri úprave registrácie nastala chyba!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(deleteRegistAPiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Úspešne si vymazal svoju registráciu!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>O niekoľko sekúnd aktualizujeme stav tvojej registrácie, <strong>ktorú si môžeš na tejto stránke skontrolovať alebo podľa potreby zaregistrovať nanovo.</strong></i>
            </div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Pri vymazaní registrácie nastala chyba!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
