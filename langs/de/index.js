  ...
  async function postData(url, data) {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "X-Original-Domain": localStorage.getItem("cd_"),
      },
  ...
  postData(getCreditApiUrl, requestCredit)
    .then((data) => {
      ...
      if (dataCredit.credit_points >= 3500) {
        qualifiedMessage.innerHTML = `<span class="text-primary-color text-center mt-6 mb-6 lh-25"><strong>Herzlichen Glückwunsch, du bist in der Lostrommel!</strong></span>`;
      }
    })
    .catch((error) => {
    ...
    .then((data) => {
      ...
      switch (dataRegist.status_id) {
        case 2:
          ...
          <div class="col-lg-4">
            <button type="submit" onclick="getActionName(this.id)" id="createRegBtn" class="btn btn-create">Anmeldung absenden</button>
          </div>

          `;
          regStatus.innerText = "Du hast dich noch nicht angemeldet";
          guestName.classList.remove("form-control-second");
          guestName.classList.add("form-control-first");
          guestBoxInfo.innerHTML = `<span class="fs-16 lh-22">Es kann nur ein Name eingegeben werden</span>`;
          gdpr.innerHTML = `
          
          <p class="small-font">Vor Ort ist die Zustimmung zu unserem <strong>Datenschutzhinweis</strong> erforderlich, insbesondere in Bezug auf die Nutzung von Bild-,
          Ton- und Videoaufnahmen, die während der Veranstaltung gemacht werden. Diesen kannst du vorab <a href="docs/nics-adatkezelesi-tajekoztato.pdf" target="_blank">hier lesen.</a></p>
          `;
          break;
        case 1:
          regActions.innerHTML = `
            ...
            <div class="">
              <button type="submit" onclick="getActionName(this.id)" id="updateRegBtn" class="btn btn-update">Anmeldung aktualisieren</button>
            </div>
            <br>
            <hr>
            <span>Falls du doch nicht zur Party kommen kannst, lösche deine Anmeldung bitte – das hilft uns bei der Organisation!</span>
            <div class="pt-20">
              <button type="submit" onclick="getActionName(this.id)" id="deleteRegBtn" class="btn btn-delete">Anmeldung löschen</button>
            </div>
          ...
          regStatus.innerText = "Du hast deine Anmeldung bereits gesendet";
          guestBoxInfo.innerHTML = `<span class="fs-16"><sup>*</sup>Der Name kann später geändert oder gelöscht werden</span>`;
          guestNameLabel.innerText = "Name deiner Begleitperson:";
        ...
        case 0:
          ...
          customerName.innerText = "Identifikation fehlgeschlagen!";
          regForm.innerHTML = ``;
        ...
        case 5:
          ...
          closedRegBox.innerHTML = `
            ...
            <span class="material-icons text-primary-color" id="regIcon">lock</span>
              <span><strong>Die Anmeldefrist für die Geburtstagsveranstaltung ist bereits abgelaufen!</strong><br>
              <span class="d-block pt-10">Wenn du dich angemeldet hast, sehen wir uns auf der Party! Falls du diesmal nicht dabei sein kannst – im Juni 2026 gibt es erneut die Möglichkeit zum Mitfeiern!
...
form.addEventListener("submit", (event) => {
  ...
  if (guestName.value.trim() !== "" && !isValidText(guestName.value)) {
    ...
    guestErrorMessage.innerHTML = `<label class"d-block pt-10 fs-16">Bitte gib nur Buchstaben ein!</label>`;
  } else {
      ...
      postData(createRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Vielen Dank, du hast dich erfolgreich angemeldet!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>In wenigen Sekunden wird deine Anmeldung aktualisiert – <strong>du kannst sie auf dieser Seite prüfen, ändern oder löschen.</strong></i>
            </div>
            
              `;
            document.getElementById("createRegBtn").disabled = true;
          } else {
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Bei der Anmeldung ist ein Fehler aufgetreten!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(updateRegistApiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Du hast deine Anmeldung erfolgreich aktualisiert!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Beim Aktualisieren der Anmeldung ist ein Fehler aufgetreten!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
      ...
      postData(deleteRegistAPiUrl, requestRegist)
        .then((data) => {
          ...
          if (dataRegist.status_id === 1) {
            ...
            <div class="alert alert-success" role="alert" id="bx">Du hast deine Anmeldung erfolgreich gelöscht!
              <br>
              <br>
              <div class="loader">
                <div class="bar"></div>
              ...
              <br>
              <i>In wenigen Sekunden wird der Status deiner Anmeldung aktualisiert – <strong>du kannst ihn hier einsehen oder dich ggf. erneut anmelden.</strong></i>
            </div>
            ...
            messageOnSubmit.innerHTML = `<span class="alert alert-danger" role="alert">Beim Löschen der Anmeldung ist ein Fehler aufgetreten!</span>`;
          }
        })
        .catch((error) => {
          console.error("Error:", error);
